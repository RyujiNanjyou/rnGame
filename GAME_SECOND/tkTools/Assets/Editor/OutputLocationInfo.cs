...
public class OutputLocationInfo : MonoBehaviour {
    ...
    {
        GameObject collision = GameObject.Find("EnemyLocation");
        ...
        foreach (Transform tr in collisions)
        {
            ...
            headerTxt += "{\n";
            headerTxt += string.Format("\t\"Assets/Model/{0}\",\n", modelName);
        ...
        StreamWriter sw = new StreamWriter(@"E:\_Users\buram_000\Desktop\GAME_SECOND\GAME_SECOND\Enemyinfo.h", false, Encoding.UTF8);//Enemyinfo.hlocationInfo.h
        sw.Write(headerTxt);
        sw.Close();
    }
