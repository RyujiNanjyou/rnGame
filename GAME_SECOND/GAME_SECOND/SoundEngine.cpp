...
#define NUM_PRESETS 30
	...
	{
		memset(hx3DAudio, 0, sizeof(hx3DAudio));
	}
	/*!
	 * @brief	デストラクタ。
	 */
	...
	void SoundEngine::Init()
	{
...
#endif
		if (FAILED(hr = XAudio2CreateReverb(&reverbEffect, flags))) {
			Release();
		...
		if (FAILED(hr = xAudio2->CreateSubmixVoice(&submixVoice, 1,
		...
		X3DAudioInitialize(details.OutputFormat.dwChannelMask, SPEEDOFSOUND, hx3DAudio);
		listener.Position = { 0.0f, 0.0f, 0.0f };
		listener.OrientFront = { 0.0f, 0.0f, 1.0f };
		listener.OrientTop = { 0.0f, 1.0f, 0.0f };
		
		/*fListenerAngle = 0.0f;
		useListenerCone = true;
		fUseInnerRadius = true;
		useRedirectToLFE = ((details.OutputFormat.dwChannelMask & SPEAKER_LOW_FREQUENCY) != 0);*/
		//初期化完了。
		isInited = true;
	}
	/*!
	...
	*/
	IXAudio2SourceVoice* SoundEngine::CreateXAudio2SourceVoice(WaveFile* waveFile, bool is3DSound)
	{
		...
		if (is3DSound == false) {
			//2Dサウンド。
			if (FAILED(xAudio2->CreateSourceVoice(&pSourceVoice, waveFile->GetFormat())))
			{
			...
			if (FAILED(xAudio2->CreateSourceVoice(&pSourceVoice, waveFile->GetFormat(), 0, 2.0f, NULL, &sendList)))
			{
				TK_LOG("Failed CreateSourceVoice");
				return nullptr;
			}
	...
	*/
	void SoundEngine::Update()
	{
		...
		if (listener.Position.x != listenerPosition.x
			...
			//動いた分を計算。
			D3DXVECTOR3 vDelta;
			vDelta = listenerPosition - listenerPos;
			fListenerAngle = float(atan2(listener.OrientFront.x, listener.OrientFront.z));

			if (useListenerCone == true) {
		...
		if (deltaTime > 0.0f) {
			//リスナーの移動速度を計算する。
			D3DXVECTOR3 vel;
			vel = listener.Position;
			vel = listenerPosition - vel;
			vel /= deltaTime;
		...
		//3Dサウンドの計算。
		for (auto& soundSource : soundSource) {
			...
			X3DAudioCalculate(hx3DAudio, &listener, &emitter, dwCalcFlags,
				dspSettings);
			...
			if (voice != nullptr)
			{
				...
				voice->SetOutputFilterParameters(masteringVoice, &FilterParametersDirect);
				...
				voice->SetOutputFilterParameters(submixVoice, &FilterParametersReverb);
			}
		}
	}